...
save_path = pathlib.Path('simulation/ardfa_ARVar_fits')
...
def run_simstudy_replicate(sample_size, theta, condition, replicate):
    '''
    Fit DFA model and save outputs for one simulation replicate
    '''

    samples_file = save_path / f'samples_{sample_size}_{theta}_{condition}_{replicate}.pkl'
    summary_file = save_path / f'summary_{sample_size}_{theta}_{condition}_{replicate}.txt'
    if samples_file.exists() and samples_file.stat().st_size > 0:
        print(f'model fit file already exists; skipping {sample_size} {theta} {condition} {replicate}')
        return
    ...
    dfa_model = ARCHDFA(num_timesteps=obs.shape[0], num_series=obs.shape[1], 
                        num_horizons=obs.shape[2],num_factors=4, p=1, q=1, 
